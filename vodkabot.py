...
def NOTIFIED_ADD_CONTACT(op):
    try:
        sendMessage(op.param1, client.getContact(op.param1).displayName + "   Salam Merc Babat Add 😘😍😍😍😍 Pagem Mano Add Kon 😍 line.me/R/ti/p/%40nlr3381q 😍")
...
def NOTIFIED_KICKOUT_FROM_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param3).displayName + " ini Ke Left Dad Jaghi Bot \n (*´･ω･*) Page Mano Add Kon : line.me/R/ti/p/%40nlr3381q ")
    except Exception as e:
        print e
...
def NOTIFIED_LEAVE_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param2).displayName + " ini Ke Left Dad Jaghi Bot \n (*´･ω･*) Page Mano Add Kon : line.me/R/ti/p/%40nlr3381q ")
    except Exception as e:
        print e
    ...
    try:
        ...
        if msg.toType == 2:
            if msg.contentType == 0:
                ...
                if msg.text == "Cancelall":
                    group = client.getGroup(msg.to)
                    if group.invitee is None:
                        sendMessage(op.message.to, "Kasi Davat Nashode.")
                    else:
                        gInviMids = [contact.mid for contact in group.invitee]
                        client.cancelGroupInvitation(msg.to, gInviMids)
                        sendMessage(msg.to, str(len(group.invitee)) + " cancel Shodan | Bara Hemayat Az ma page Mano Add Konid = line.me/R/ti/p/%40nlr3381q")
                if "invite:" in msg.text:
                    key = msg.text[-33:]
                    client.findAndAddContactsByMid(key)
                    client.inviteIntoGroup(msg.to, [key])
                    contact = client.getContact(key)
                    sendMessage(msg.to, ""+contact.displayName+" I Ye Jaghi Davat Dadam")
                if msg.text == "me":
                    M = Message()
                    M.to = msg.to
                    M.contentType = 13

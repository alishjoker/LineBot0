...
def NOTIFIED_ADD_CONTACT(op):
    try:
        sendMessage(op.param1, client.getContact(op.param1).displayName + " salam Mrc Babat Add Like Kon Liike Shi BOT :) Add Me http://line.me/R/ti/p/%40nlr3381q")
...
def NOTIFIED_ACCEPT_GROUP_INVITATION(op):
    #print op
    try:
        sendMessage(op.param1, client.getContact(op.param2).displayName + "Welcome To Group :| " + group.name)
    except Exception as e:
        print e
...
def NOTIFIED_KICKOUT_FROM_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param3).displayName + " B ~ K \n khob ke chi Left Midi 5Min Dige Bayad Davat Beshi Kon Goshad ( add Kon Page Mano = line.me/R/ti/p/%40nlr3381q")
    except Exception as e:
        print e
    ...
    try:
        if msg.toType == 0:
            if msg.contentType == 0:
                ...
                if msg.text == "gift":
                    sendMessage(msg.to, text="http://line.me/R/shop/detail/1635361", contentMetadata=None, contentType=9)
                else:
                    pass
            else:
                pass
            ...
            if msg.contentType == 0:
                ...
                if msg.text == "cn":
                    ...
                    if group.invitee is None:
                        ...
                        sendMessage(msg.to, str(len(group.invitee)) + " Davat Dashti bz Ro Link line.me/R/ti/p/%40nlr3381q")
                if "invite:" in msg.text:
                    key = msg.text[-33:]
                    client.findAndAddContactsByMid(key)
                    client.inviteIntoGroup(msg.to, [key])
                    contact = client.getContact(key)
                    sendMessage(msg.to, ""+contact.displayName+" I Koni hast Age Nayad")
                if msg.text == "man":
                    M = Message()
                    M.to = msg.to
                    M.contentType = 13
